...
NTLMStrategy.prototype._validateUser = function(ntlmSSPMessage, req, successCallback) {
    ...
    activeSmb.authenticate(ntlmSSPMessage.ntlm, ntlmSSPMessage.lm,
        ...
        function(authenticated) {
            self._smbServers.push(req.session.smbServer); // Make the SMB server available again
            if (authenticated) {
    ...
    function exec() {
        if (self._smbServers) {
            ...
            smb.getChallenge(function(challenge) {
                ...
                req.session.smbServer = smbServer;
                debug('SMB Challenge: ' + req.session.challenge);
