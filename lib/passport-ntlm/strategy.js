...
function NTLMStrategy(options, verify) {
  ...
  this._dnsDomain=options.dnsDomain;
  this._smbServers=[];
  if (this._dnsDomain || this._domain) {
	self=this;
	
	if (! this._dnsDomain) {
		this._dnsDomain=this._domain + '.com';
	}
	this._queue = [];
	...
	if (!options.smbServer) { throw new TypeError('NTLM Strategy requires dnsDomain, domain or smbServer to be configured'); 
  }
