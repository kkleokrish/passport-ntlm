...
NTLMSSPMessage.prototype.buildType2Message= function(challenge, targetServerInformation) {
  ...
  var ntlmType2MessagePart1=new Buffer(48);
  ntlmType2MessagePart1.write(this.signature,0,null,'ascii');
  ntlmType2MessagePart1.writeUInt32LE(this.ntlmType);
  
  var targetNameSecurityBuffer=new SecurityBuffer();
  targetNameSecurityBuffer.data(new Buffer(this.targetInformation.domain,SecurityBuffer.DATATYPE.UCS2),48);
  ...
  var targetInformationArray=[];
  if (this.targetServerInformation.server)  {
	var serverBuffer = new Buffer(this.targetServerInformation.server, 'ucs2');
	var targetInformationBuffer=new Buffer(4+serverBuffer.length);
  ...
  if (this.targetServerInformation.domain)  {
	var domainBuffer = new Buffer(this.targetServerInformation.domain, 'ucs2');
	var targetInformationBuffer=new Buffer(4+domainBuffer.length);
  ...
  if (this.targetServerInformation.serverDNS)  {
	var serverDNSBuffer = new Buffer(this.targetServerInformation.serverDNS, 'ucs2');
	var targetInformationBuffer=new Buffer(4+serverDNS.length);
	targetInformationBuffer.writeUInt16LE(2,0);
	targetInformationBuffer.writeUInt16LE(serverDNS.length,2);
	serverDNS.copy(targetInformationBuffer,4);
	targetInformationArray.push(targetInformationBuffer);
  }  

  if (this.targetServerInformation.domainDNS)  {
	var domainDNSBuffer = new Buffer(this.targetServerInformation.domainDNS, 'ucs2');
	var targetInformationBuffer=new Buffer(4+domainDNS.length);
	targetInformationBuffer.writeUInt16LE(2,0);
	targetInformationBuffer.writeUInt16LE(domainDNS.length,2);
	domainDNS.copy(targetInformationBuffer,4);
	targetInformationArray.push(targetInformationBuffer);
  }  
  
  var terminationBuffer=new Buffer(4);
  terminationBuffer.writeUInt16LE(0,0);
  terminationBuffer.writeUInt16LE(0,2);
  targetInformationArray.push(terminationBuffer);
  
  var targetInformationArrayBuffer=Buffer.concat(targetInformationArray);
  
  
  var targetInformationSecurityBuffer=new SecurityBuffer();
  targetInformationSecurityBuffer.data(targetInformationArrayBuffer,40 + targetNameSecurityBuffer.length);
  
  targetInformationSecurityBuffer.toBuffer().copy(ntlmType2MessagePart1,40);

  console.log(respAuthHex);
  ntlmSspType2Message.buffer=Buffer.concat([ntlmType2MessagePart1,targetNameSecurityBuffer.extractData(SecurityBuffer.DATATYPE.BUFFER),targetInformationArrayBuffer]);
